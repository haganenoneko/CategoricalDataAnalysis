ocAME < - function (w, rev.dum = TRUE, digits = 3) {
    ...
    pfun < -
        switch (w$method, probit = pnorm, logistic = plogis, loglog = p1gumbel, cloglog = p1Gumbel)
    dfun < -
        switch (w$method, probit = dnorm, logistic = dlogis, loglog = d1gumbel, cloglog = d1Gumbel)
    V2 < -vcov(w)
    V3 < -rbind(cbind(V2, 0, 0), 0, 0)
    ind < -c(1: K, nrow(V3) - 1, (K + 1): (K + J - 1), nrow(V3))
    V4 < -V3[ind, ]
    V5 < -V4[, ind]# 3. Calculate average marginal effects(AME)# 3.1 AME value
    vec1 < -rep(1, nrow(x))
    ...
    colnames(me) < -paste("effect", lev[c(1, J)], sep = ".")# 3.2 AME standard error
